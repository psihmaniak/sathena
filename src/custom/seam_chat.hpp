...
// line is validated in clif_process_message (name/message are MUTABLE buffers → rewrite in place;
// return false to BLOCK the line). onChatDeliver fires at the top of clif_GlobalMessage (area
// delivery). onLogChat taps every chat line at the log_chat sink. Defaults = pass-through / no-op
// => vanilla chat. A
