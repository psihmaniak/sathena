...
#ifdef SATHENA
	// [SATHENA-SEAM] custom SC id range — the consumer's <custom/sc_enum.inc> defines ids
	// with EXPLICIT high values (e.g. SC_X = 2000) so upstream insertions above never shift
	// them (no scdata/icon drift on rebase). SC_MAX lands ABOVE the block automatically, so
	// SC_MAX-sized arrays and <SC_MAX guards cover custom ids with zero surgery. The stub
	// ships empty (vanilla SC_MAX unchanged). PLACEMENT: last entries before SC_MAX.
	#include <custom/sc_enum.inc>
#endif
...
#ifdef SATHENA
	// [SATHENA-SEAM] custom EFST id range — the consumer's <custom/efst_enum.inc> defines
	// ids with EXPLICIT high values (e.g. EFST_X = 2000); each needs a matching client
	// StateIconImgInfo lub entry to render. EFST_MAX lands ABOVE the block so
	// EFST_MAX-sized arrays (StatusRelevantBLTypes) auto-cover custom ids. Stub ships
	// empty. PLACEMENT: after @APIHOOK_END (the API generator owns everything above),
	// before EFST_MAX.
	#include <custom/efst_enum.inc>
#endif
